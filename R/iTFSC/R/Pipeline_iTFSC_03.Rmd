...
##### Homework 3, running SCENIC pipeline ######################
# Utilizing SCENIC package to infer transcriptic factor activity
...
# Libraries for downstream visualizations
...
dbDir <- "/Users/monicabonilla/Desktop/scenic_test/"

# Subsetting the data , 100 cells per cell type . 
# Read in Mahnoor's RDS file. This won't be needed if you run script from beginning.
#Qian_merged_interest_downsample <- readRDS(Qian_merged_interest_downsample.RDS)

merged_data_combined <- SetIdent(Qian_merged_interest_downsample, value = Qian_merged_interest_downsample@meta.data$cell_types )
down.sampleMG <- subset(merged_data_combined,  downsample = 100)

# save RDS to your machine 
saveRDS(down.sampleMG, "/Users/Desktop/monicabonilla/scenic_test/down.sampleMG.RDS")

########################

exprMat <- as.matrix(down.sampleMG@assays$RNA@counts)
dim(exprMat)

cellInfo <- as.data.frame(down.sampleMG@meta.data$cell_types)
cellInfo <- data.frame(seuratCluster=Idents(down.sampleMG))


# run the rest of the pipeline now. 

myDatasetTitle <- "data_comp_scenic"

# Should be your directory 
dbDir <- "/Users/monicabonilla/Desktop/scenic"
data(defaultDbNames)
dbs <- defaultDbNames[[org]]

# download files online to your local computer/great lakes 
...
saveRDS(scenicOptions, file="int/scenicOptions.Rds") # To save statu

### Run SCENIC pipeline on cluster###########


scenicOptions <- readRDS("/Users/monicabonilla/Desktop/scenic/scenicOptions.Rds")
...
# save visualization files to a directory 
png(file="/Users/monicabonilla/Desktop/scenic/scenic.png", width=1000, height=4000)
ComplexHeatmap::Heatmap(regulonActivity_byCellType_Scaled, name="Regulon activity")
dev.off()

regulonActivity_byCellType_Scaled_df <- as.data.frame(regulonActivity_byCellType_Scaled)
regulonActivity_byCellType_Scaled_df$gene <- rownames(regulonActivity_byCellType_Scaled_df)
SREBF2 <- filter(regulonActivity_byCellType_Scaled_df, gene == "SREBF2 (99g)")

SREBF2$gene <- NULL
png(file="/Users/monicabonilla/Desktop/scenic/scenic_SRB.png", width=1000, height=500)
ComplexHeatmap::Heatmap(SREBF2, name="Regulon activity")
dev.off()
